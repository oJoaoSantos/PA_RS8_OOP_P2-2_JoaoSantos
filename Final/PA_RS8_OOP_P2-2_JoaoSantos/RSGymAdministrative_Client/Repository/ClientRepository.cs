        ...
        public static void ReadClient()
        {
            using (var context = new _DatabaseContext())
            {
                var readClient = context.ZipCode.Join(context.Client,
                                                    ...
                                                    .ToList();
                readClient.ToList().ForEach(c => Console.WriteLine($" * ID: {c.ClientID} * Estado: {(c.ActiveNow == true ? "Ativo" : "Inativo")} * NIF: {c.ClientVat} * Data de Nascimento: {c.BirthDate.ToShortDateString()} * Nome: {c.ClientName} * Telemóvel: {c.ClientPhoneNumber} * Email: {c.ClientEmail} * Morada: {c.ClientAdress}, {c.Zip} - {c.City} * Observações: {c.ClientObservations} *"));                
            }
        }
        #endregion
